...
EXISTS(SELECT 1 FROM vw_batch_history h WHERE m.batch_code = h.batch_code AND m.batch_attempt = h.attempt)
GROUP BY source_id),
rec_chg AS(
...
LEFT OUTER JOIN max_cutoff_date ON trim(max_cutoff_date.source_id) = trim(rec_chg.source_id)
